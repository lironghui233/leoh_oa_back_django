...
const onSubmitEditStaff = async () => {
    ...
    try{
        let new_staff = await staffHttp.updateStaffStatus(staff.uid, staffForm.status)
        ElMessage.success('员工状态修改成功！')
        dialogVisible.value = false
        staffs.value.splice(handleIndex, 1, new_staff)
    }catch(detail){
...
const onSearch = () => {
    fetchStaffList(1, page_size.value, filterForm)
}

const onDownload = async () => {
    let rows = tableRef.value.getSelectionRows()
    if(!rows || rows.length==0){
        ElMessage.info("请先选择要到导出的员工！")
        return
    }
    try{
        ...
        // 模拟点击行为，只要点击了，浏览器启动下载操作（下载href属性指定的数据）
        a.click()
...
async function fetchStaffList(page, page_size, filterForm) {
    try{
        // 获取员工列表
